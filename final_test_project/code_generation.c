...
int generate_io_library() {
    ...
    const char *io_library_tm_code = "ioLibrary.tm";
...
void generate_load_parameter_constant(TreeNode *tree, SymbolTable *symbol_table) { 
    ...
    if (tree->oper != 0 && tree->oper != NOT && tree->oper != CHSIGN) {
        /* LHS constant param */
...
void generate_load_constant(TreeNode *tree, SymbolTable *symbol_table) {
    ...
    if (tree->type == Integer) {
        ...
        load_comment = "Load integer char constant";
    }
    else if (tree->type == CharInt) {
        load_comment = "Load charint constant";
    }
...
void generate_if_then_leave(TreeNode *tree, SymbolTable *symbol_table) {
    ...
    int then_pc;
    if (tree->child[1] != NULL) {
        then_pc = tree->child[1]->program_counter_address ;
    }

    int jump_past = emitWhereAmI() - then_pc;
...
void generate_if_else_leave(TreeNode *tree, SymbolTable *symbol_table) {
    ...
    if (tree->child[2] != NULL) {
        else_pc = tree->child[2]->program_counter_address;
    }

    int jump_past = emitWhereAmI() - else_pc - 1;
...
void generate_for_statement_leave(TreeNode *tree, SymbolTable *symbol_table) {
    ...
    int compound_start_jump = (emitWhereAmI() - compound_node->program_counter_address) + 1;
    emitRM("JMP", AC3, (compound_start_jump + compound_node->dataword_size) * -1, AC3, "go to beginning of loop");
    emitNewLoc(compound_node->program_counter_address - 1);
    emitRM("JMP", AC3, compound_start_jump, AC3, "Jump past loop [backpatch]");
    emitSkip(compound_start_jump);
...
void generate_range_statement_save_start(TreeNode *tree, SymbolTable *symbol_table) {
    emitRM("ST", RT, tree->mem_offset * -1, GP, "save starting value in index variable");
}

void generate_range_statement_save_stop(TreeNode *tree, SymbolTable *symbol_table) {
    emitRM("ST", RT, (tree->mem_offset + 1) * -1, GP, "save stop value"); 
}

void generate_range_statement_save_step(TreeNode *tree, SymbolTable *symbol_table) {
    emitRM("ST", RT, (tree->mem_offset + 2) * -1, GP, "save step value"); 
}
