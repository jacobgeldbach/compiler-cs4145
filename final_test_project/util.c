        ...
        if (tree->nodekind==StmtK) {
            ...
            switch (tree->subkind.exp) {
                case OpK:
                    ...
                    else if ((tree->attr.op == CHSIGN || tree->attr.op == SIZEOF) && tree->child[1] != NULL) {
                ...
                case AssignK:
                    ...
                    if ((tree->attr.op == ADDASS || tree->attr.op == ASGN) && tree->child[0] != NULL) {
                        tree->child[0]->oper = tree->attr.op;
                    }
        ...
        for (i=0;i<MAXCHILDREN;i++) {
            if (tree->child[i] != NULL) {
                    ...
                    if (if_statement_active) {
                        switch (i) {
                            case 0: // simple exp skip
                                break;
                            case 1: // then part
                                append_linearized_node(tree->child[1], CG_EnterThen, current_head);
                                then_part = get_current_tail_node(current_head);
                                break;
                            case 2: // else part
                                append_linearized_node(tree->child[2], CG_EnterElse, current_head);
                ...
                if (tree->nodekind == StmtK && tree->subkind.stmt == RangeK) {
                    switch (i) {
                            case 0: 
                                append_linearized_node(last_for_node[last_for_node_idx - 1]->ast_node->child[0], CG_RangeSaveStart, current_head);
                                break;
                             case 1: 
                                append_linearized_node(last_for_node[last_for_node_idx - 1]->ast_node->child[0], CG_RangeSaveStop, current_head);
                                break;
                            case 2:
                                append_linearized_node(last_for_node[last_for_node_idx - 1]->ast_node->child[0], CG_RangeSaveStep, current_head);
                                break;
                            default:
                                break;
                    }
        ...
        if (tree->nodekind==StmtK) {
            switch (tree->subkind.stmt) {
                case IfK: {
                    ...
                    if (else_part != NULL) {
                        code_generation_linearized_node_t *end_then_part = initialize_temp_list();
                        append_linearized_node(tree, CG_LeaveThen, end_then_part);
                        insert_linearized_node(end_then_part, else_part->prev);
            ...
            switch (tree->subkind.exp) {
                ...
                case IdK:
                    ...
                    if (tree->isParam && !expect_lhs_var && call_params_expected) {
                        ...
                        /* Makes sure we dont count an expression as two siblings */
                        last_param_sibling_num = sibling;
                    ...
                    if (expect_lhs_var) {
                        ...
                        if (tree->isArray) {
                            ...
                            if (tree->oper == DIV || tree->oper == TIMES || tree->oper == OR || tree->oper == AND || tree->oper == PLUS 
                                    || tree->oper == LT || tree->oper == MOD || tree->oper == LEQ || tree->oper == GT || tree->oper == MINUS || tree->oper == EQ) {
                                append_linearized_node(tree, CG_AssignLHSPush, current_head);
                            }
                ...
                case CallK:
                    ...
                    if (tree->attr.op == SIZEOF || tree->attr.op == MOD || tree->oper == PLUS || tree->oper == MINUS || tree->oper == DIV || tree->oper == TIMES) {
                        if (tree->oper == TIMES || tree->oper == EQ || tree->oper == PLUS || tree->oper == MINUS || tree->oper == DIV) {
                            append_linearized_node(tree, CG_AssignLHSPush, current_head);
                        }
                    }
