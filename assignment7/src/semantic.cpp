...
char *errors_found[5000];
int errors_found_line_num[5000];
int errors_found_idx = 0;
int num_errors = 0;    /* Number of errors */
int num_warnings = 0;  /* Number of warnings */

int goffset = 0;        /* global offset for variable memory locations */
int foffset = 0;        /* function offset for variable memory locations */

int foffset_saved_stack[100];       /* stack that keeps track of the last foffset before entering a new scope */
int foffset_saved_stack_idx = 0;    /* current index for saved_foffset_stack, incremented when a new scope is entered, decremented when exiting a scope */

int func_decl_param_size = 0;
...
void semantic_analysis(TreeNode * tree, SymbolTable * symbol_table) {
    ...
    while (tree != NULL) {
        ...
        if (tree->nodekind == StmtK && tree->subkind.stmt == CompoundK) {
            foffset_saved_stack[foffset_saved_stack_idx++] = foffset;       /* Save the current foffset on the stack before entering a new scope */
            ...
            while ((param_sibling != NULL) && (((param_sibling->nodekind == DeclK) && (param_sibling->subkind.decl == ParamK)) || (param_sibling->nodekind == DeclK && param_sibling->subkind.decl == VarK))) {
                ...
                if (symbol_table->insert(param_sibling->attr.name, (void *) param_sibling) == false) {
                    ...
                    if (param_sibling->subkind.decl == ParamK) {
                        param_sibling->mem_offset = foffset;
                        param_sibling->var_kind = Parameter;
                        foffset += param_sibling->dataword_size;

                        func_decl_param_size++;
                    }
                }
        ...
        if (tree->nodekind == DeclK && tree->subkind.decl == FuncK) {
            ...
            if (symbol_table->insertGlobal(tree->attr.name, (void *) tree) == false) {
                ...
                tree->var_kind = Global;
                tree->mem_offset = 0;
                tree->dataword_size = 2; /* Functions take 2 datawords for return address and static link */
                foffset = 0; /* Reset function offset for each new function */
                foffset += tree->dataword_size;
                tree->return_address = tree->mem_offset+1; /* The return address is at the current foffset */
            } 
        }
        ...
        if (tree->nodekind==StmtK) {
            switch (tree->subkind.stmt) {
                ...
                case CompoundK:
                    ...
                    tree->var_kind = None;
                    tree->dataword_size = foffset; /* The size of the compound stmt is the current foffset */
                    tree->mem_offset = 0;
                ...
                case ForK:
                    ...
                    tree->mem_offset = 0;
                    foffset += 2;
                    tree->dataword_size = foffset; 
                    
                    if (tree->child[2] != NULL && tree->child[0]->subkind.decl == VarK && tree->child[2]->subkind.stmt == CompoundK) {
                        TreeNode *decl_node = (TreeNode *)symbol_table->lookup(tree->child[0]->attr.name);
                        if (decl_node != NULL) {
                            decl_node->isUsed = true;
                        }
            ...
            switch (tree->subkind.exp) {
                ...
                case ConstantK:
                    ...
                    /* Check if this initializer is a STRINGCONST or Char array, must allocate the memory globally. Global Character Arrays  */
                    if (tree->type == Char && tree->isArray == true) {
                        tree->var_kind = Global;
                        tree->mem_offset = goffset + 1;
                        tree->dataword_size = strlen(tree->attr.string) - 2 + 1; /* -2 for quotes, +1 for null terminator */
                            
                        goffset += tree->dataword_size;
                    }
                    else {
                        tree->dataword_size = 1; /* Constants take 1 dataword */
                    }
                ...
                case IdK: {
                    ...
                    if (decl_node != NULL) {
                        ...
                        /* Memory information from when the memory for this variable was allocated on its declaration */
                        tree->var_kind = decl_node->var_kind;
                        tree->mem_offset = decl_node->mem_offset;
                        tree->dataword_size = decl_node->dataword_size;
                        ...
                        decl_node->isArray = tree->isArray;
                        decl_node->isUsed = true;
                ...
                case CallK: 
                    ...
                    if (tree->nodekind == ExpK && tree->subkind.exp == CallK) {
                        ...
                        if (decl_node != NULL) {
                            ...
                            /* Memory information from when this function was declared and its memory allocated */
                            tree->var_kind = decl_node->var_kind;
                            tree->mem_offset = decl_node->mem_offset;
                            tree->dataword_size = decl_node->dataword_size;
                            ...
                            while (p != NULL && q != NULL) {
                                ...
                                if ((p->attr.op == LBRACKET || p->attr.op == LEQ || p->attr.op == GT || p->attr.op == EQ || p->attr.op == AND || p->attr.op == OR || p->attr.op == NEQ || p->attr.op == LT ||
                                            p->attr.op == GEQ || p->attr.op == NOT || p->attr.op == PLUS || p->attr.op == MINUS || p->attr.op == CHSIGN || p->attr.op == TIMES || p->attr.op == DIV ||
                                                p->attr.op == MOD || p->attr.op == ASGN || p->attr.op == ADDASS || p->attr.op == SUBASS || p->attr.op == MULASS || p->attr.op == DIVASS || p->attr.op == INC || p->attr.op == DEC)
                                                && p->child[0] != NULL) {
                                    p->child[0]->isParam = true;
                                    p->isParam = true;

                                    TreeNode *tmp = p->child[0];
                                    while (tmp->nodekind == ExpK && tmp->subkind.exp == OpK && (tmp->attr.op == LBRACKET || tmp->attr.op == MINUS || tmp->attr.op == TIMES || tmp->attr.op == PLUS ||
                                                tmp->attr.op == DIV || tmp->attr.op == MOD || p->attr.op == ASGN || p->attr.op == OR || p->attr.op == AND) 
                                                    && tmp->child[0] != NULL) {
                                        tmp = tmp->child[0];
                                        tmp->isParam = true;
                                    }
                                    ...
                                    tmp = p->child[0];
                                    if ((tmp->attr.op == CHSIGN || tmp->attr.op == SIZEOF || tmp->attr.op == TERNARY) && tmp->child[1] != NULL) { /* Unary rel op params*/
                                        tmp->child[1]->isParam = true;
                                        if (p->child[1]->nodekind == ExpK && p->child[1]->subkind.exp == OpK && (p->child[1]->attr.op == LBRACKET || p->child[1]->attr.op == MINUS) && p->child[1]->child[0] != NULL) {
                                            p->child[1]->child[0]->isParam = true;
                                        }
                                    }
                                ...
                                if ((p->attr.op == CHSIGN || p->attr.op == SIZEOF || p->attr.op == TERNARY) && p->child[1] != NULL) { /* Unary rel op params*/
                                        p->child[1]->isParam = true;
                                        if (p->child[1]->nodekind == ExpK && p->child[1]->subkind.exp == OpK && (p->child[1]->attr.op == LBRACKET || p->child[1]->attr.op == MINUS) && p->child[1]->child[0] != NULL) {
                                            p->child[1]->child[0]->isParam = true;
                                        }
                                }
                            ...
                            if (p != NULL && q == NULL) {
                                    ...
                                    /* Unary rel op params */
                                    if ((p->attr.op == CHSIGN || p->attr.op == SIZEOF || p->attr.op == TERNARY) && p->child[1] != NULL) {
                                        printf("HERE\n");
                                        p->child[1]->isParam = true;
                                        if (p->child[1]->nodekind == ExpK && p->child[1]->subkind.exp == OpK && (p->child[1]->attr.op == LBRACKET || p->child[1]->attr.op == MINUS) && p->child[1]->child[0] != NULL) {
                                            p->child[1]->child[0]->isParam = true;
            ...
            switch (tree->subkind.decl) {
                case FuncK: {
                    ...
                    decl_node->dataword_size += func_decl_param_size; /* Add the size of the parameters to the function decl dataword_size */;
                    func_decl_param_size = 0; /* Reset for next function decl */
                ...
                case VarK:
                    ...
                    if (tree->var_kind == Local) {
                        if (tree->isArray) {
                            tree->mem_offset = foffset + 1;
                        }
                        else {
                            tree->mem_offset = foffset;
                        }
        ...
        if (scope_entered) {
            ...
            foffset = foffset_saved_stack[--foffset_saved_stack_idx]; /* Decrement the foffset_stack_idx, and restore the foffset value to what it was before entering this scope that is being exited */
            scope_entered = false;
        }
...
TreeNode * init_io_library_ast() {
    ...
    output_int_param->dataword_size = 1; /* Integer parameter is dataword in size */
    ...
    output_bool_param->dataword_size = 1; /* Boolean parameter is dataword in size */
    ...
    output_char_param->dataword_size = 1; /* Char parameter is dataword in size */
    ...
    input_func_decl->dataword_size = 2; /* Input function is 2 dataword in size */
    input_func_decl->program_counter_address = 1; /* Input function starts at address 1 */
    ...
    input_b_func_decl->dataword_size = 2; /* Inputb function is 2 dataword in size */
    input_b_func_decl->program_counter_address = 12; /* Input function starts at address 12 */
    ...
    input_c_func_decl->dataword_size = 2; /* Inputc function is 2 dataword in size */
    input_c_func_decl->program_counter_address = 23; /* Input function starts at address 23 */
    ...
    output_func_decl->dataword_size = 2; /* Output function is 2 dataword in size */
    output_func_decl->program_counter_address = 6; /* Output function starts at address 6 */
    ...
    output_b_func_decl->dataword_size = 3; /* Outputb function is 2 dataword in size */
    output_b_func_decl->program_counter_address = 17; /* Outputb function starts at address 17 */
    ...
    output_c_func_decl->dataword_size = 3; /* Outputc function is 2 dataword in size */
    output_c_func_decl->program_counter_address = 28; /* Outputc function starts at address 28 */
    ...
    output_nl_func_decl->dataword_size = 2; /* Outnl function is 2 dataword in size */
    output_nl_func_decl->program_counter_address = 34; /* Outnl function starts at address 34 */
